    ...
    <div>
        <h4 class="modal-title product-title">
            <a class="product-title2" href="{{ route('product', $product->slug) }}" data-toggle="tooltip"
                ...
                <i
                    class="czi-arrow-{{ Session::get('direction') === 'rtl' ? 'left' : 'right' }} ms-2 font-size-lg mr-0"></i>
            </a>
        </h4>
    </div>
    <div>
        ...
        <div class="col-lg-5 col-md-4 col-12">
            <div class="cz-product-gallery position-relative">
                ...
                <div class="table-responsive">
                    <div class="d-flex">
                        <div id="sync2"
                            class="owl-carousel owl-theme product-thumb-slider max-height-100px d--none">
                            @if ($product->images != null && json_decode($product->images) > 0)
                                @if (json_decode($product->colors) && $product->color_image)
                                    @foreach (json_decode($product->color_image) as $key => $photo)
                                        @if ($photo->color != null)
            ...
            <div class="details __h-100">
                ...
                <form id="add-to-cart-form" class="mb-2">
                    ...
                    <div class="__btn-grp align-items-center mb-2">
                            ...
                            ($product->added_by == 'seller' &&
                                ($seller_temporary_close ||
                                    (isset($product->seller->shop) &&
                                        ...
                                        $currentDate >= $seller_vacation_start_date &&
                                        $currentDate <= $seller_vacation_end_date))) ||
                                ($product->added_by == 'admin' &&
                                    ($inhouse_temporary_close ||
                                        ($inHouseVacationStatus &&
                                            $currentDate >= $inhouse_vacation_start_date &&
                                            $currentDate <= $inhouse_vacation_end_date))))
                        ...
                        @else
                            <button class="btn btn-secondary action-buy-now-this-product data_orderNow" type="button"
                                data-id="{{ $product['id'] }}" data-name="{{ $product['name'] }}"
                                data-price="{{ $product['unit_price'] }}">
                                {{ translate('buy_now') }}
                            </button>
                            <button class="btn btn--primary string-limit action-add-to-cart-form data_addToCard"
                                type="button" data-update-text="{{ translate('update_cart') }}"
                                data-add-text="{{ translate('add_to_cart') }}" data-id="{{ $product['id'] }}"
                    ...
                    <div class="row no-gutters d-none flex-start d-flex">
                        <div class="col-12">
                            @if ($product['product_type'] == 'physical')
                                <h5 class="text-danger out-of-stock-element d--none">{{ translate('out_of_stock') }}
                                </h5>
                            @endif
                        </div>
                    </div>
