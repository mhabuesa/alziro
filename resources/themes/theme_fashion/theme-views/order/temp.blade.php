                                    ...
                                    @if ($cart_key == 0)
                                        <div class="--bg-6 border-0 rounded py-2 px-2 px-sm-3 ">
                                            <div class="d-flex flex-wrap align-items-center justify-content-between ">
                                                ...
                                                @php
                                                    $verify_status = \App\Utils\OrderManager::minimum_order_amount_verify(
                                                        $request,
                                                        $group_key,
                                                    );
                                                @endphp
                                                ...
                                                @if ($physical_product && $shippingMethod == 'sellerwise_shipping' && $shipping_type == 'order_wise')
                                                    @php
                                                        $choosen_shipping = \App\Models\CartShipping::where([
                                                    ...
                                                    @php
                                                        $shippings = \App\Utils\Helpers::get_shipping_methods(
                                                            $cartItem['seller_id'],
                                                            $cartItem['seller_is'],
                                                        );
                                    ...
                                    <form class="cart add_to_cart_form{{ $cartItem['id'] }}"
                                        ...
                                        <table class="table __table vertical-middle cart-list-table-custom">
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        <input type="text" name="id" value="{{ $cartItem->id }}"
                                                            hidden>
                                                        ...
                                                        <div class="cart-product  align-items-center">
                                                            ...
                                                            <div
                                                                class="cont {{ $product->status == 0 ? 'blur-section' : '' }}">
                                                                                ...
                                                                                <div class="d-flex column-gap-1">
                                                                                    ...
                                                                                    <select
                                                                                        ...
                                                                                        @foreach ($choice->options as $value)
                                                                                            <option
                                                                                                value="{{ trim($value) }}"
                                                                                                {{ in_array(trim($value), $variations, true) ? 'selected' : '' }}>
                                                                                                {{ ucwords($value) }}
                                                                                            </option>
                                                                                        @endforeach
                                                                                    </select>
                                                    ...
                                                    <td class="text-center">
                                                        @if ($cartItem['discount'] > 0)
                                                            <span
                                                                class="badge badge-soft-base product_discount{{ $cartItem['id'] }}">-{{ \App\Utils\Helpers::currency_converter($cartItem['discount'] * $cartItem['quantity']) }}</span>
                                                        @else
                                                            <span
                                                        ...
                                                        @if ($minimum_order)
                                                            <div class="quantity __quantity">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div class="quantity__minus cart-qty-btn updateCartQuantityList_cart_data"
                                                                        data-minorder="{{ $minimum_order->minimum_order_qty }}"
                                                                        data-cart="{{ $cartItem['id'] }}" data-value="-1"
                                                                        data-action="{{ $cartItem['quantity'] == $minimum_order->minimum_order_qty ? 'delete' : 'minus' }}">
                                                                        <i
                                                                            class="{{ $cartItem['quantity'] == (isset($cartItem->product->minimum_order_qty) ? $cartItem->product->minimum_order_qty : 1) ? 'bi bi-trash3-fill text-danger fs-10' : 'bi bi-dash-lg' }}"></i>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        @else
                                ...
                                @if ($free_delivery_status['status'] && (session()->missing('coupon_type') || session('coupon_type') != 'free_delivery'))
                                    <div class="free-delivery-area px-3 mb-3">
                                        <div class="d-flex align-items-center gap-3">
                                            <img loading="lazy" src="{{ theme_asset('assets/img/free-shipping.svg') }}"
                            ...
                            @foreach ($cart as $group_key => $group)
                                        ...
                                        <div class="--bg-6 border-0 rounded py-2 px-2 px-sm-3 ">
                                            <div
                                                ...
                                                @if ($physical_product && $shippingMethod == 'sellerwise_shipping' && $shipping_type == 'order_wise')
                                                    ...
                                                    @if ($physical_product && $shippingMethod == 'sellerwise_shipping' && $shipping_type == 'order_wise')
                                                        <div
                                                            class="max-sm-100px bg-white select-method-border rounded  py-2">
                                                            <div class="d-flex overflow-hidden">
                                                                <div
                                                                    class="flex-middle flex-nowrap fw-semibold text-dark mx-2">
                                                                    <i class="bi bi-truck"></i>
                                                                </div>
                                ...
                                @foreach ($group as $key => $cartItem)
                                    ...
                                    <form class="cart add_to_cart_form{{ $cartItem['id'] }}"
                                        ...
                                        @csrf
                                        <div
                                            class="d-flex gap-3 pb-3 border-bottom justify-content-between align-items-center">
                                                                ...
                                                                @if (!empty(json_decode($product->colors)))
                                                                    <div class="d-flex column-gap-1">
                                                                        ...
                                                                        <select
                                                                            ...
                                                                            @foreach (json_decode($product->colors) as $k => $value)
                                                                                <option
                                                                                    value="{{ $value }}"{{ $cartItem->color == $value ? 'selected' : '' }}>
                                                                                    {{ \App\Utils\get_color_name($value) }}
                                                                                </option>
                                                                            @endforeach
                                                                        </select>
                                            ...
                                            @if ($minimum_order)
                                                <div
                                                    class="quantity quantity--style-two d-flex flex-column align-items-center">
                                                    <div class="quantity__minus cart-qty-btn updateCartQuantityListMobile_cart_data"
                                                        data-minorder="{{ $minimum_order->minimum_order_qty }}"
                                                        data-cart="{{ $cartItem['id'] }}" data-value="-1"
                                                        data-action="{{ $cartItem['quantity'] == $minimum_order->minimum_order_qty ? 'delete' : 'minus' }}">
                                                        <i
                                                    ...
                                                    <div class="quantity__plus cart-qty-btn updateCartQuantityListMobile_cart_data"
                                                        data-minorder="{{ $minimum_order->minimum_order_qty }}"
                                                        data-cart="{{ $cartItem['id'] }}" data-value="1"
                                                        data-action="">
                                                        <i class="bi bi-plus "></i>
                        ...
                        <div class="total-cost-wrapper">
                            ...
                            <div class="total-cost-area">
                                    ...
                                    <form action="javascript:" method="post" novalidate id="coupon-code-ajax">
                                        <div class="apply-coupon-form">
                                            <input type="text" class="form-control" name="code" id="promo-code"
                                                placeholder="{{ translate('apply_coupon_code') }}" required
                                                autocomplete="off">
                                            <button class="btn badge-soft-base"
                                                id="coupon_code_theme_fashion">{{ translate('apply') }}</button>
                                        </div>
                                ...
                                <div class="proceed-cart-btn">
                                    ...
                                    <button
                                        class="btn btn-base w-100 justify-content-center mt-1 form-control h-42px text-capitalize checkout_action"
